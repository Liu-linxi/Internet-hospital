				...
				<swiper-item class="picker-calendar" v-for="(calendar,calendarIndex2) in calendars"
					...
					<view class="picker-calendar-view" v-for="(date,dateIndex) in calendar" :key="dateIndex"
						...
						<view class="picker-calendar-view-item"
							:style="{opacity: date.statusStyle.opacity, color: date.disabled ?  'rgb(221, 221, 221)' : date.statusStyle.color, background: date.statusStyle.background}">
					...
					<block v-if="isMultiSelect">
						<view class="picker-display">
							...
							<view v-if="isContainTime" class="picker-display-link" :hover-stay-time="100"
								...
								@click.stop="onShowTimePicker('begin')">{{BeginTimeTitle}}</view>
						</view>
						<view class="picker-display">
							...
							<view v-if="isContainTime" class="picker-display-link" :hover-stay-time="100"
								hover-class="picker-display-link-active" :style="{color}"
								@click.stop="onShowTimePicker('end')">{{EndTimeTitle}}</view>
						</view>
					</block>
					<block v-else>
						<view class="picker-display">
		...
		<view v-if="showTimePicker" class="picker">
			<view class="picker-modal picker-time">
				...
				<view class="picker-modal-footer">
					...
					<view class="picker-modal-footer-btn">
						<view class="picker-btn" :hover-stay-time="100" hover-class="picker-btn-active"
							@click.stop="onCancelTime">取消</view>
						<view class="picker-btn" :style="{color}" :hover-stay-time="100" hover-class="picker-btn-active"
							@click.stop="onConfirmTime">确认</view>
					</view>
				</view>
			</view>
		</view>
		...
		getCalendar(date, proc, max, min) {
			...
			for (let i = 0; i < 42; i++) {
				...
				if (max) {
					...
					// 当前日期可选
					disabled = it.getTime() > new Date(max).getTime() + 86400000
				}
				if (min) {
		...
		methods: {
			...
			refreshCalendars(refresh = false) {
				...
				if (this.calendarIndex == 0) {
					if (refresh) this.calendars.splice(0, 1, DateTools.getCalendar(date, this.procCalendar, this.max, this
						.min));
					...
					if (refresh) this.calendars.splice(1, 1, DateTools.getCalendar(date, this.procCalendar, this.max, this
						.min));
					...
					if (refresh) this.calendars.splice(2, 1, DateTools.getCalendar(date, this.procCalendar, this.max, this
						.min));
				}
