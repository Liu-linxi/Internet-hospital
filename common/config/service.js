...
import Request from '@/common/config/luch-request/index.js'
 ...
 * 修改全局配置示例
 const test = new Request({
	header: {a:1}, // 举例
	baseURL: $serviceURI.webUrl,
	validateStatus: (statusCode) => { // statusCode 必存在。此处示例为全局默认配置
		return statusCode >= 200 && statusCode < 300
	}
})
 test.config.baseURL = $serviceURI.webUrl
 **/

test.setConfig((config) => { /* 设置全局配置 */
  config.baseURL = $serviceURI.webUrl
  config.header = {
    ...config.header,
    a: 1, // 演示
    b: 2 // 演示
...
http.setConfig((config) => { /* 设置全局配置 */
  config.baseURL = $serviceURI.webUrl /* 根域名不同 */
  config.header = {
    ...config.header,
    a: 1, // 演示
    b: 2 // 演示
